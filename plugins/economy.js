...
smd({
    ...
    async(message) => {
      try{
       ...
       await eco.deduct(users, "QUEEN_KYLIE", balance.wallet);
...
async(message,match) => {
  try{
   ...
   const balance  = await eco.balance(user, "QUEEN_KYLIE")
   switch (value) {
       case '1000':
       case '1':
       if (k > balance.wallet ) return message.reply(`*_You need to pay 🪙100 to increase bank capacity ~ 1000 sp_*`);
         const deduct1 = await eco.deduct(user, "QUEEN_KYLIE", 100);
         const add1 = eco.giveCapacity(user,"QUEEN_KYLIE", 1000);
       ...
       if (k < balance.wallet) return message.reply(`*You need to pay 🪙1000 to increase bank capacity ~ 100000 sp*`);
         const deduct2 = await eco.deduct(user,"QUEEN_KYLIE", 1000);
         const add2 = eco.giveCapacity(user, "QUEEN_KYLIE", 100000);
